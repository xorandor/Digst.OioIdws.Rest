...
namespace Digst.OioIdws.Rest.AuthorizationService.Tests
{
    ...
    {
        [TestMethod]
        public async Task Authorize_Success_ReturnsCorrectly()
        {
        ...
        [TestMethod]
        public async Task Authorize_OtherEndpoint_PassesThrough()
        {
            using (var server = TestServer.Create(app =>
            {
                app
        ...
        [TestMethod]
        public async Task Authorize_InvalidRequest_ReturnsUnauthorized()
        {
            var authorizationServiceMock = new Mock<IAccessTokenGenerator>();
            var tokenStoreMock = new Mock<ITokenStore>();
