...
namespace Digst.OioIdws.Rest.AuthorizationService
{
    public class OioIdwsAuthorizationServiceMiddleware : OwinMiddleware
    {
        ...
        private static Settings ValidateOptions(OioIdwsAuthorizationServiceOptions options)
        {
            ...
            try
            {
                settings.AuthorizationPath = new PathString(options.AuthorizationEndpoint);
            }
            catch (Exception ex)
            {
                throw new ArgumentException("'AuthorizationEndpoint' must be set to a valid relative url where the endpoint will be hosted", ex);
            }
